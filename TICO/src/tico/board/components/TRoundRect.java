...
/**
 * Defines a <code>TRoundRect</code> and its attributes.
 *
 * @author Pablo Muñoz
 * @version 1.0 Nov 20, 2006
 */
public class TRoundRect extends TComponent {
	/**
	 * The <code>TRoundRect</code> type used as attribute in XML encoding and decoding.
	 */
	public static String TYPE = "roundRect";
	
	private static int roundRectCount = 0;

	/**
	 * Creates a new <code>TRoundRect</code>.
	 */
	public TRoundRect() {
		super();
	 ...
	 */
	public String newId() {
    	return "roundRect_" + new Integer(roundRectCount++);
	}
	...
	/**
	 * Returns a <code>TRoundRect</code> object from the data contained in
	 * the XML <code>Element</code>.
	 * 
	 * @param element The XML <code>Element</code> that contains the round rectangle data
	 * @return The generated <code>TRoundRect</code>
	 * @throws InvalidFormatException If <code>Element</code> has an invalid format
	 */
	public static TRoundRect XMLDecode(Element element) throws InvalidFormatException {
		if (element.getTagName().equals("component")
				&& element.getAttribute("type").equals(TYPE)) {
			// Create the empty round rectangle
			TRoundRect roundRect = new TRoundRect();
			// Get the attributes
			Node attributesNode = element.getElementsByTagName("attributes")
					.item(0);
			// Apply attributes to the round rectangle
			roundRect.getAttributes().applyMap(TAttributeEncoder
