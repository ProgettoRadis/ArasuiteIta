/*
 * File: TRectangleMarqueeHandler.java
 ...
 * board, a new <code>rectangle</code> of the selected dimension.
 * 
 * @author Pablo Muñoz
 * @version 1.0 Nov 20, 2006
 */
public class TRectangleMarqueeHandler extends TComponentMarqueeHandler {
	/**
	 * Creates a new <code>TRectangleMarqueeHandler</code> for the specified
	...
	protected TComponent createDefaultComponent(Rectangle2D bounds) {
		TComponent rectangle = new TRectangle();

		rectangle.getAttributes().applyMap(
				getBoardContainer().getEditor().getCurrentAttributes());

		TBoardConstants.setBounds(rectangle.getAttributes(), bounds);
