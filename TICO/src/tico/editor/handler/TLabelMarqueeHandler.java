 ...
 */
public class TLabelMarqueeHandler extends TComponentMarqueeHandler {
	/**
	...
	protected TComponent createDefaultComponent(Rectangle2D bounds) {
		TComponent label = new TLabel();

		label.getAttributes().applyMap(
				getBoardContainer().getEditor().getCurrentAttributes());

		TBoardConstants.setBounds(label.getAttributes(), bounds);
