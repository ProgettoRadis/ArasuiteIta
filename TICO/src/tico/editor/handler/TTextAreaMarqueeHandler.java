/*
 * File: TTextAreaMarqueeHandler.java
 ...
 * board, a new <code>textArea</code> of the selected dimension.
 * 
 * @author Pablo Muñoz
 * @version 1.0 Nov 20, 2006
 */
public class TTextAreaMarqueeHandler extends TComponentMarqueeHandler {
	/**
	 * Creates a new <code>TTextAreaMarqueeHandler</code> for the specified
	...
	protected TComponent createDefaultComponent(Rectangle2D bounds) {
		TComponent textArea = new TTextArea();

		textArea.getAttributes().applyMap(
				getBoardContainer().getEditor().getCurrentAttributes());

		TBoardConstants.setBounds(textArea.getAttributes(), bounds);
